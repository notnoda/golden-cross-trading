...
from market.conf.config import Config
...
# API POST 호출
################################################################################
async def post(config: Config, path, params, name="Out"):
    res = requests.post(
...
# 현재가 조회
################################################################################
async def inquiry_price(config: Config, stock_code: str):
...
# 틱봉 조회
################################################################################
async def chart_tick(config: Config, stock_code: str, tick_size: int) -> DataFrame:
...
# 주식 주문
################################################################################
async def order(config: Config, stock_code: str, tp_code: str, order_price: str, order_qty=1):
...
# 주문 취소
################################################################################
async def cancel_order(config: Config, stock_code: str, order_no: str):
    path = "/api/v1/trading/overseas-stock/order"
    params = json.dumps({
        "In": {
            "AstkIsuNo" : stock_code,
...
# 체결내역조회
################################################################################
async def transaction_history(config: Config, stock_code: str, order_no=None):
    path = "/api/v1/trading/overseas-stock/inquiry/transaction-history"
    params = json.dumps({
        "In": {
            "QrySrtDt": config.start_date, #조회시작일자
            "QryEndDt": config.end_date, #조회종료일자
...
# 해외주식 잔고/증거금 조회
################################################################################
async def inquiry_balance(config: Config, name="Out"):
...
async def inquiry_balance_qty(config: Config, stock_code=None):
    path = "/api/v1/trading/overseas-stock/inquiry/balance-margin"
    params = json.dumps({
        "In": {
            "TrxTpCode": "2", #처리구분코드(1:외화잔고, 2:주식잔고상세, 3:주식잔고(국가별), 9:당일실현손익)
